...
int main() {
    ...
    Plane* plane1 = new Plane("SCE", "EWR", globalFlightMap);
    plane1->setVel(450);
    ...
    pilot1.myPlane = plane1;
    std::cout << "Pilot " << pilot1.getName() << " has boarded the plane. Plane memory address: " << pilot1.myPlane << std::endl;
    ...
    for (int i = 0; i < 15000; i += 15) {
        ...
        if (plane1->getAt_SCE()) {
            ...
            std::cout << "Current Pilot: " << (pilot1.getName() == "Pilot-in-Command" ? "Pilot-in-Command" : "Co-Pilot")
                << ", Plane Memory Address: " << (pilot1.myPlane ? pilot1.myPlane : pilot2.myPlane) << std::endl;
        }
    ...
    // Delete the dynamically allocated plane
    delete plane1;
